...
type BillableEntryWriter interface {
	// CreateBillableOperation(operationID billing_entities.BillableOperationKey, planID uuid.UUID, payableID *uuid.UUID, subscriptionID uuid.UUID, amount float64, args map[string]interface{}, rxn shared.ResourceOwner) error
	Create(ctx context.Context, billableOperation *billing_entities.BillableEntry) (*billing_entities.BillableEntry, error)
}

type SubscriptionWriter interface {
	// CreateSubscription(planID uuid.UUID, args map[string]interface{}, rxn shared.ResourceOwner) (*billing_entities.Subscription, error)
